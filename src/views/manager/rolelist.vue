<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-button v-has-add:uac_roles class="filter-item" style="margin-left: 10px;" @click="handleCreate" type="primary" icon="el-icon-edit">添加</el-button>
    </div>
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="努力加载中" border fit highlight-current-row
      ...
      <el-table-column align="center" label="操作" class-name="small-padding fixed-width" width="300px">
        <template slot-scope="scope">
          <el-button v-has-perm:uac_roles_perms type="primary" size="mini" @click="handleOpenPermDialog(scope.row)">权限</el-button>
          <el-button v-has-update:uac_roles v-show="scope.row.status=='0'" type="primary" size="mini" @click="handleUpdate(scope.row)">编辑</el-button>
          <el-button v-has-status:uac_roles v-show="scope.row.status!='-1'" size="mini" type="success" @click="handleModifyStatus(scope.row,'-1')">删除
          </el-button>
          <el-button v-has-status:uac_roles v-show="scope.row.status!='0'" size="mini" @click="handleModifyStatus(scope.row,'0')">正常
          </el-button>
          <el-button v-has-status:uac_roles v-show="scope.row.status!='1'" size="mini" type="danger" @click="handleModifyStatus(scope.row,'1')">锁定
          </el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    handleOpenPermDialog(row) {
      ...
      getRolePerms(row.id).then(response => {
        const rolePerms = response.result.map(x => {
          return x.id
        })
      ...
      return (
        <span style='flex: 1; display: flex; align-items: center; font-size: 14px; padding-right: 8px;'>
          ...
          <span>
            ...
            <span class='treenode'>{data.action}</span>
